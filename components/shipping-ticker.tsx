...
export function ShippingTicker() {
  const messages = ["FREE SHIPPING ON ORDERS OVER $200", "30-DAY RETURNS", "AUTHENTIC LOCAL CRAFTSMANSHIP"]
      ...
      <div className="flex gap-8">
        ...
        <div className="flex items-center gap-8 whitespace-nowrap animate-marquee-infinite">
          {messages.map((message, index) => (
            <span
              key={`set1-${index}`}
              className="inline-flex items-center gap-1.5 text-[10px] md:text-xs font-medium uppercase tracking-wide"
            >
